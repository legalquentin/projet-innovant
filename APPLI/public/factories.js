...
app.factory('Service_get', function($http, $q) {
  var dataInfo = "";
  function fetchData(param) {
    ...
    if (_isDataLoaded()) {
      ...
      $http.get('http://localhost:433/'+param)
...
app.factory('Service_post', function($http, $q) {
  var dataInfo = "";
  function sendData(param) {
    ...
    if (_isDataLoaded()) {
      ...
      $http.post('http://localhost:433/'+param.path, param.payload)
      .success(function(response) {
        deffered.resolve(response);
        var data = deffered.dataInfo;
      });
